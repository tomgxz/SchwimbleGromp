        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 0: # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength: # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    await ctx.send(embed=CooldownEmbed(ctx=ctx,message=f"send your chicken to fight",remainingTime=cooldownLength-cooldowndiff).embed)
                    return
            ...
            userChickens = csvToList(self.db.getUserSetting(user.id,guild.id,"inventory_chicken"))

            if len(userChickens) < 1:
                await ctx.send(embed=ErrorEmbed(ctx=ctx,message="You need to buy a chicken from the store first using `buy chicken`").embed)
                return
            ...
            if chicken.strength >= opponent.strength:
                if opponent.strength > chicken.strength//2: chicken.strength -= opponent.strength//12
                ...
                await ctx.send(embed=Embed(ctx=ctx,type=EmbedType.success,message=f"Your lil chicken won the fight, and made you {bet} {coinname} richer! :rooster:",footer=f"Your Chicken's New Strength (used to determine winner): {chicken.strength}").embed)
                await self.newWalletBalance(ctx)
            else:
                # repickle dead chicken
                emoji=getEmoji("rip")
                self.db.updateUserBalance(user.id,guild.id,bet*-1,"wallet")
                await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"Your chicken died {ripEmoji}").embed)
                await self.newWalletBalance(ctx)
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 0: # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength: # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    await ctx.send(embed=CooldownEmbed(ctx=ctx,message=f"play blackjack",remainingTime=cooldownLength-cooldowndiff).embed)
                    return
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 0: # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength: # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    await ctx.send(embed=CooldownEmbed(ctx=ctx,message=f"use the slot machine",remainingTime=cooldownLength-cooldowndiff).embed)
                    return
            ...
            if final[0]==final[1] or final[1]==final[2] or final[0]==final[2]: # if two or more are the same
                if final.count(":skull:") > 1: # if more than one is a skull
                    if final[0] == final[1] == final[2]: # if all are skulls
                        ...
                        await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You rolled three skulls and lost {humanize.humanizeNumber(bet*4)} {coinname}",fields=f,inline=True).embed)
                    else: # if two are skulls
                        self.db.updateUserBalance(user.id,ctx.guild.id,bet * -2,"wallet")
                        await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You rolled two skulls and lost {humanize.humanizeNumber(bet*2)} {coinname}",fields=f,inline=True).embed)
                elif ":star:" in final and final[0] == final[1] == final[2]: # if all are stars
                        self.db.updateUserBalance(user.id,ctx.guild.id,bet * 4,"wallet")
                        await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You rolled three stars and gained {humanize.humanizeNumber(bet*4)} {coinname}",fields=f,inline=True).embed)
                elif final[0] == final[1] == final[2]: # if all are the same
                    self.db.updateUserBalance(user.id,ctx.guild.id,bet * 3,"wallet")
                    await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You rolled three identical items and gained {humanize.humanizeNumber(bet*3)} {coinname}",fields=f,inline=True).embed)
                else: # if two are the same
                    self.db.updateUserBalance(user.id,ctx.guild.id,bet * 2,"wallet")
                    await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You rolled two identical items and gained {humanize.humanizeNumber(bet*2)} {coinname}",fields=f,inline=True).embed)
            else: # if none are the same
                self.db.updateUserBalance(user.id,ctx.guild.id,bet * -1,"wallet")
                await ctx.send(embed=ErrorEmbed(ctx=ctx,message=f"You didn't role anything good and lost {humanize.humanizeNumber(bet)} {coinname}",fields=f,inline=True).embed)
