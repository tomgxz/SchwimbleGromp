...
class Economy(commands.Cog):
    ...
    async def beg(self,ctx):
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(user.id,guild.id,"cooldowns_beg",wrapQuotes(cooldown.currentCooldownTime()))

            # COMMAND HANDLING

            p_earnings=[];weights=[];j=10

            for i in range(20):
                for l in range(5): weights.append(j)
                j-=1
    ...
    async def rob(self,ctx,victim:discord.Member=None):
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(user.id,guild.id,"cooldowns_rob",wrapQuotes(cooldown.currentCooldownTime()))
    ...
    async def crime(self,ctx):
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(user.id,guild.id,"cooldowns_crime",wrapQuotes(cooldown.currentCooldownTime()))
    ...
    async def slut(self,ctx):
        ...
        async with ctx.typing():
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(user.id,guild.id,"cooldowns_slut",wrapQuotes(cooldown.currentCooldownTime()))
