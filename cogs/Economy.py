...
class Economy(commands.Cog):
        ...
        async with ctx.typing():
            ...
            if user == None:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx, message="You need to specify a user").embed)
                return
            ...
            if store not in ["wallet", "bank"]:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message="You need to specify a money store (wallet or bank)"
                ).embed)
                return

            if amount == None:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx, message="You did not specify an amount").embed)
                return
    ...
    async def setusercooldown(self,
        ...
        async with ctx.typing():
            ...
            self.db.setUserSetting(
                ...
                f"commandsUntilCooldownRemaining_{command}",
                self.db.getGuildSetting(ctx.guild.id,
                                        f"commandsUntilCooldown_{command}"))
            self.db.setUserSetting(
                user.id, ctx.guild.id, f"cooldowns_{command}",
                self.db.getGuildSetting(ctx.guild.id, f"cooldowns_{command}"))

        await ctx.send(embed=Embed(
            ctx=ctx,
            type=EmbedType.success,
            message=f"You reset {user.display_name}'s {command} cooldown").
                       embed)
    ...
    async def leaderboard(self, ctx, amount=None):
        ...
        async with ctx.typing():
            coinname = self.db.getGuildSetting(ctx.guild.id, "coinname")
            if amount == None:
                amount = self.db.getGuildSetting(ctx.guild.id,
                                                 "defaultLeaderboardEntries")
            if not amount.isdigit():
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message="`Amount` needs to be a whole number").embed)
                return
            lb = {}
            total = []
            amount = int(amount)
            ...
            for amt in total:
                f.append([
                    f"{index}. {self.bot.get_user(lb[str(amt)]).display_name}",
                    f"{humanize.humanizeNumber(amt)} {coinname}"
                ])
                if index == amount:
                    changeAmt = False
                    break
        ...
        await ctx.send(embed=Embed(ctx=ctx,
                                   type=EmbedType.passive,
                                   message=f"Top {amount} Richest People",
                                   fields=f).embed)
    ...
    async def withdraw(self, ctx, amount=None):
        ...
        async with ctx.typing():
            ...
            if bal < 0:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message="You can't withdraw while you are in debt").embed)
                return
            elif bal == 0:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=
                    f"You can't withdraw without any {coinname} in your bank").
                               embed)
                return
            ...
            if amount > bal:
                await ctx.send(embed=BankAmountInsufficientEmbed(
                    ctx=ctx, coinname=coinname).embed)
                return
            if amount <= 0:
                await ctx.send(embed=InvalidIntEmbed(ctx=ctx).embed)
                return
            ...
            self.db.updateUserBalance(user.id, ctx.guild.id, -1 * amount,
                                      "bank")
            ...
            e = Embed(
                ...
                type=EmbedType.success,
                message=
                f"You withdrew {humanize.humanizeNumber(amount)} {coinname}"
            ).embed
        ...
        commandsUntilCooldownRemaining = self.db.getUserSetting(
            user.id, ctx.guild.id, "commandsUntilCooldownRemaining_withdraw")
    ...
    async def deposit(self, ctx, amount=None):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                ctx.guild.id, "commandsUntilCooldown_deposit")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, ctx.guild.id,
                "commandsUntilCooldownRemaining_deposit")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, ctx.guild.id, "firstCommandExecuted_deposit")
            cooldownLength = self.db.getGuildSetting(ctx.guild.id,
                                                     "cooldowns_deposit")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, ctx.guild.id,
                                           "firstCommandExecuted_deposit",
                                           '""')

                else:  # if the cooldown is still active stop the command
                    await ctx.send(
                        embed=CooldownEmbed(ctx=ctx,
                                            message=f"deposit any {coinname}",
                                            remainingTime=cooldownLength -
                                            cooldowndiff).embed)
                    return
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, ctx.guild.id, "cooldowns_deposit",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            if bal < 0:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message="You can't deposit while you are in debt").embed)
                return
            elif bal == 0:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=
                    f"You can't deposit without any {coinname} in your wallet"
                ).embed)
                return
            if amount.lower() == "all": amount = bal
            ...
            e = Embed(
                ...
                type=EmbedType.success,
                message=
                f"You deposited {humanize.humanizeNumber(amount)} {coinname}"
            ).embed
        ...
        # make sure the amount of allowed commands remaining is decreased
        commandsUntilCooldownRemaining = self.db.getUserSetting(
            user.id, ctx.guild.id, "commandsUntilCooldownRemaining_deposit")
        if commandsUntilCooldownRemaining > 0:  # decrease cooldown value by 1
            self.db.setUserSetting(
                user.id, ctx.guild.id,
                "commandsUntilCooldownRemaining_deposit",
    ...
    async def send(self, ctx, sendto: discord.Member = None, amount=None):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                ctx.guild.id, "commandsUntilCooldown_send")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, ctx.guild.id, "commandsUntilCooldownRemaining_send")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, ctx.guild.id, "firstCommandExecuted_send")
            cooldownLength = self.db.getGuildSetting(ctx.guild.id,
                                                     "cooldowns_send")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    self.db.setUserSetting(
                        user.id, ctx.guild.id,
                        "commandsUntilCooldownRemaining_send",
                        commandsUntilCooldown)
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, ctx.guild.id,
                                           "firstCommandExecuted_send", '""')

                else:  # if the cooldown is still active stop the command
                    await ctx.send(embed=CooldownEmbed(
                        ctx=ctx,
                        message=f"give anyone any more {coinname}",
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, ctx.guild.id, "cooldowns_send",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            if amount.lower() == "all":
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message="You cannot send someone all of your money yet").
                               embed)
                return
            ...
            e = Embed(
                ...
                type=EmbedType.success,
                message=
                f"You sent {humanize.humanizeNumber(amount)} {coinname} to {sendto.display_name}"
            ).embed
    ...
    async def work(self, ctx):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                guild.id, "commandsUntilCooldown_work")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, guild.id, "commandsUntilCooldownRemaining_work")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, guild.id, "firstCommandExecuted_work")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    self.db.setUserSetting(
                        user.id, guild.id,
                        "commandsUntilCooldownRemaining_work",
                        commandsUntilCooldown)
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, guild.id,
                                           "firstCommandExecuted_work", '""')
            ...
            if firstCommandExecuted != "":
                if cooldown.cooldownDiff(
                        ...
                        guild.id, "commandsUntilCooldownResetTime"
                ):  # if the time since the first command is greater than the command reset window time
                    self.db.setUserSetting(
                        user.id, guild.id,
                        "commandsUntilCooldownRemaining_work",
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, guild.id, "cooldowns_work",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            work = random.choice(replies.work()).replace(
                "{amount}", f"{humanize.humanizeNumber(earning)} {coinname}")

            self.db.updateUserBalance(user.id, guild.id, earning, "wallet")

        await ctx.send(
            embed=Embed(ctx=ctx, type=EmbedType.success, message=work).embed)
        await self.newWalletBalance(ctx)
    ...
    async def beg(self, ctx):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                guild.id, "commandsUntilCooldown_beg")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, guild.id, "commandsUntilCooldownRemaining_beg")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, guild.id, "firstCommandExecuted_beg")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    self.db.setUserSetting(
                        user.id, guild.id,
                        "commandsUntilCooldownRemaining_beg",
                        commandsUntilCooldown)
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, guild.id,
                                           "firstCommandExecuted_beg", '""')
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, guild.id, "cooldowns_beg",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            earning = random.choices([x for x in range(100)],
        ...
        await ctx.send(embed=Embed(
            ...
            type=EmbedType.success,
            message=
            f"You gained {humanize.humanizeNumber(earning)} {coinname} from begging"
        ).embed)
        await self.newWalletBalance(ctx)
    ...
    async def rob(self, ctx, victim: discord.Member = None):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                guild.id, "commandsUntilCooldown_rob")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, guild.id, "commandsUntilCooldownRemaining_rob")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, guild.id, "firstCommandExecuted_rob")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, guild.id,
                                           "firstCommandExecuted_rob", '""')

                else:  # if the cooldown is still active stop the command
                    await ctx.send(
                        embed=CooldownEmbed(ctx=ctx,
                                            message=f"rob anyone",
                                            remainingTime=cooldownLength -
                                            cooldowndiff).embed)
                    return
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, guild.id, "cooldowns_rob",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            if victim.id == user.id:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx, message="You cannot rob yourself").embed)
                return
            ...
            if amount <= 0:
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=f"{victim.name} does not have any money on them").
                               embed)
            ...
            if random.randint(1, 100) >= self.failrate(guild.id, "rob"):
                ...
                self.db.updateUserBalance(user.id, guild.id, -1 * fine,
                                          "wallet")
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=
                    f"You were caught by the police while robbing {victim.display_name} and were fined {humanize.humanizeNumber(fine)} {coinname}"
                ).embed)
                await self.newWalletBalance(ctx)
                commandsUntilCooldownRemaining = self.db.getUserSetting(
                    user.id, guild.id, "commandsUntilCooldownRemaining_rob")
            ...
            self.db.updateUserBalance(user.id, guild.id, earning, "wallet")
            self.db.updateUserBalance(victim.id, guild.id, earning * -1,
                                      "wallet")
        ...
        await ctx.send(embed=Embed(
            ...
            type=EmbedType.success,
            message=
            f"You robbed {humanize.humanizeNumber(earning)} {coinname} from {victim.name} and got away with it."
        ).embed)
        await self.newWalletBalance(ctx)
    ...
    async def crime(self, ctx):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                guild.id, "commandsUntilCooldown_crime")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, guild.id, "commandsUntilCooldownRemaining_crime")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, guild.id, "firstCommandExecuted_crime")
            cooldownLength = self.db.getGuildSetting(guild.id,
                                                     "cooldowns_crime")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    ...
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, guild.id,
                                           "firstCommandExecuted_crime", '""')

                else:  # if the cooldown is still active stop the command
                    await ctx.send(
                        embed=CooldownEmbed(ctx=ctx,
                                            message=f"commit a crime",
                                            remainingTime=cooldownLength -
                                            cooldowndiff).embed)
                    return
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, guild.id, "cooldowns_crime",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            if random.randint(1, 100) >= self.failrate(guild.id, "crime"):
                ...
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=
                    f"You were caught by the police while committing a crime and were fined {humanize.humanizeNumber(fine)} {coinname}"
                ).embed)
                await self.newWalletBalance(ctx)
                commandsUntilCooldownRemaining = self.db.getUserSetting(
                    user.id, guild.id, "commandsUntilCooldownRemaining_crime")
            ...
            crime = random.choice(replies.crime()).replace(
                "{amount}", f"{humanize.humanizeNumber(earning)} {coinname}")

            self.db.updateUserBalance(user.id, guild.id, earning, "wallet")

        await ctx.send(
            embed=Embed(ctx=ctx, type=EmbedType.success, message=crime).embed)
        await self.newWalletBalance(ctx)
    ...
    async def slut(self, ctx):
        ...
        async with ctx.typing():
            ...
            commandsUntilCooldown = self.db.getGuildSetting(
                guild.id, "commandsUntilCooldown_slut")
            commandsUntilCooldownRemaining = self.db.getUserSetting(
                user.id, guild.id, "commandsUntilCooldownRemaining_slut")
            firstCommandExecuted = self.db.getUserSetting(
                user.id, guild.id, "firstCommandExecuted_slut")
            cooldownLength = self.db.getGuildSetting(guild.id,
                                                     "cooldowns_slut")
            ...
            if commandsUntilCooldownRemaining == 0:  # if the cooldown is active (at 0)
                ...
                if cooldowndiff >= cooldownLength:  # if the cooldown has completed, reset the cooldown stats and run the command
                    self.db.setUserSetting(
                        user.id, guild.id,
                        "commandsUntilCooldownRemaining_slut",
                        commandsUntilCooldown)
                    commandsUntilCooldownRemaining = commandsUntilCooldown
                    self.db.setUserSetting(user.id, guild.id,
                                           "firstCommandExecuted_slut", '""')
            ...
            if commandsUntilCooldownRemaining == 1:
                self.db.setUserSetting(
                    user.id, guild.id, "cooldowns_slut",
                    wrapQuotes(cooldown.currentCooldownTime()))
            ...
            if random.randint(1, 100) >= self.failrate(guild.id, "slut"):
                ...
                await ctx.send(embed=ErrorEmbed(
                    ctx=ctx,
                    message=random.choice(replies.slutFail()).replace(
                        "{amount}",
                        f"{humanize.humanizeNumber(fine)} {coinname}")).embed)
                await self.newWalletBalance(ctx)
                commandsUntilCooldownRemaining = self.db.getUserSetting(
                    user.id, guild.id, "commandsUntilCooldownRemaining_slut")
                if commandsUntilCooldownRemaining > 0:  # decrease cooldown value by 1
            ...
            message = random.choice(replies.slutSuccess()).replace(
                "{amount}", f"{humanize.humanizeNumber(earning)} {coinname}")

            self.db.updateUserBalance(user.id, guild.id, earning, "wallet")

        await ctx.send(
            embed=Embed(ctx=ctx, type=EmbedType.success, message=message).embed
        )
        await self.newWalletBalance(ctx)
    ...
    async def userInputError(self, ctx, error):
        if isinstance(error, discord.ext.commands.errors.MemberNotFound):
            await ctx.send(embed=ErrorEmbed(
                ctx=ctx,
                message=f"The user `{error.argument}` was not found").embed)
